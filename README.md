...
- **星力頁**：
  - **機率成本表**：全星階 成功／維持／破壞 機率（可切換原始值／常駐 ×1.05 加成後）、各裝等每次強化費用、破壞後救回（restart／restore）對照；下方另有**期望花費表**（純強化基準：不含卷軸、道具不計價，可選消費階級）——「逐項」檢視（預設）列出各星 n→n+1 的首達期望（含破壞退回後重爬的全部費用）；「累計」檢視列出自 0★ 到每個目標星的期望楓幣與道具數（＝逐項各列相加）。
  - **強化策略計算**：輸入裝等、起始／目標星、道具市價、消費階級（強化費折扣，僅套用 16★ 以下）、卷軸，以「期望總花費最小化」（政策疊代＋線性方程組直接解）求解——每星自動在 正常強化／防止破壞 擇優、破壞後自動在 重新開始／復原 擇優，輸出期望花費分項與逐星建議表。另以蒙地卡羅模擬（固定亂數種子、依期望強化次數自動調整 run 數）呈現**花費 PR 分布**：PR 10–99 百分位、依「該趟破壞次數」堆疊上色的直方圖（單色深淺 ordinal 色階；左側多峰即破壞 0／1／2 次的花費群）、P50／P90／期望值參考線、輸入實際花費反查 PR 並在圖上標記。
